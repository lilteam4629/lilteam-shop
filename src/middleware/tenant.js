...
const { ensureLabRainModule } = require('../services/system-modules');
...
async function tenantResolver(req, res, next) {
  ...
  try {
    ...
    // Restore a LAB-only module removed by an older migration. Once a release
    // exists (including an explicit disabled release), its state is respected.
    ...
    if (shop.isSystemLab && tenantDb.settings?.systemModules?.rangersMarketPreview?.version !== '1.0.2') {
      await store.runInTenant(shop.id, tenantDb, () => store.transact(data => {
        data.settings ||= {};
        data.settings.storefrontModel = 'rangers-market';
        data.settings.systemModules ||= {};
        data.settings.systemModules.rangersMarketPreview = {
          installed: true,
          enabled: true,
          version: '1.0.2',
