(function(){
  var observer=null;
  function init(){
    var pageSurface=document.querySelector('body.admin-site main.admin-page-surface');
    var selector='main > section,main > article,main > div,.admin-content > section,.admin-content > article,.admin-content > div';
    var nodes=pageSurface?[pageSurface]:Array.from(document.querySelectorAll(selector)).filter(function(node){return !node.closest('[role="dialog"]')});
