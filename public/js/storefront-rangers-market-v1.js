(()=>{
  ...
  if(!root)return;
  const grid=root.querySelector('#rm-grid'),cards=[...grid.children],search=root.querySelector('#rm-search'),stock=root.querySelector('#rm-stock'),sort=root.querySelector('#rm-sort'),limit=root.querySelector('#rm-limit'),hideSold=root.querySelector('#rm-hide-sold'),count=root.querySelector('#rm-count'),filterCount=root.querySelector('#rm-filter-count'),empty=root.querySelector('#rm-empty');
  function render(){
    const q=search.value.trim().toLocaleLowerCase('th'),mode=stock.value;
    let visible=cards.filter(c=>(!q||c.dataset.title.includes(q))&&(mode==='all'||c.dataset.stock===mode)&&(!hideSold?.checked||c.dataset.stock!=='sold'));
