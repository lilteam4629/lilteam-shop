(()=>{
  ...
  function render(){
    ...
    let visible=cards.filter(c=>(!selectedCodes.length||selectedCodes.every(code=>(c.dataset.rangerCodes||'').split(',').includes(code)))&&(!tokens.length||tokens.every(t=>c.dataset.title.includes(t)))&&(mode==='all'||c.dataset.stock===mode)&&(!hideSold?.checked||c.dataset.stock!=='sold'));
