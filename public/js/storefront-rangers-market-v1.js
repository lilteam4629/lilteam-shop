(()=>{
    ...
    const selectedFilter=selectedCodes.length
      ? c=>{
        ...
        return matchMode?.value==='any'
          ? selectedCodes.some(code=>assignedCodes.includes(normalizeCode(code)))
          : selectedCodes.every(code=>assignedCodes.includes(normalizeCode(code)));
      }
      : ()=>false;
  ...
  search.addEventListener('input',render,{passive:true}); stock.addEventListener('change',render); sort.addEventListener('change',render); limit?.addEventListener('change',render); hideSold?.addEventListener('change',render); matchMode?.addEventListener('change',render);
  root.addEventListener('ranger-selection',e=>{selectedCodes=e.detail?.codes||[];render()});
  root.querySelector('#rm-reset').addEventListener('click',()=>{search.value='';stock.value='all';sort.value='default';if(limit)limit.value='20';if(hideSold)hideSold.checked=false;if(matchMode)matchMode.value='all';render()});
  root.addEventListener('click',e=>{const button=e.target.closest('[data-rm-rangers-nav]');if(!button)return;const strip=button.closest('.rm-row-rangers-viewport')?.querySelector('[data-rm-rangers-strip]');if(strip)strip.scrollBy({left:(button.dataset.rmRangersNav==='next'?1:-1)*Math.max(120,strip.clientWidth*.8),behavior:'smooth'})});
  render();
})();
