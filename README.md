...
**1. สร้างเว็บใหม่ให้ผู้เช่า (ทำจากมือถือได้ผ่าน Railway dashboard)**
- สร้างโปรเจกต์ Railway ใหม่จาก repo นี้ (Deploy from GitHub repo)
- ตั้งค่า Environment Variables อย่างน้อย: `ADMIN_USERNAME`, `ADMIN_PASSWORD` (ของผู้เช่า), `MONGODB_URI` (แนะนำ Atlas free tier ดูวิธีด้านบน)
- ถ้าจะบังคับให้ต้องมีคีย์ก่อนใช้งานได้ (ดูข้อ 2) ให้ตั้ง `LICENSE_SECRET` เป็นค่าลับที่คุณกำหนดเอง (ใช้ค่าเดียวกันได้กับทุกเว็บที่คุณปล่อยเช่า หรือจะแยกคนละค่าต่อผู้เช่าก็ได้)
- Deploy เสร็จได้ URL เว็บร้านใหม่ ส่งให้ผู้เช่า

**2. ระบบคีย์ปลดล็อก (ไม่บังคับ — ถ้าไม่ตั้ง `LICENSE_SECRET` เว็บจะใช้งานได้ปกติไม่มีการล็อก)**

ถ้าตั้ง `LICENSE_SECRET` ไว้ เว็บจะรีไดเรกต์ทุกหน้าไปที่ `/license` จนกว่าจะกรอกคีย์ที่ถูกต้อง คีย์ตรวจสอบในตัวเว็บเองด้วยลายเซ็นดิจิทัล (HMAC) ไม่ต้องมีเซิร์ฟเวอร์กลาง ไม่ต้องต่อเน็ตกลับมาหาคุณ

ออกคีย์ให้ผู้เช่าด้วยคำสั่ง (รันจากเครื่องคุณเอง หรือให้ Claude รันให้ในเซสชันที่มีโค้ดนี้):

```bash
LICENSE_SECRET=ค่าลับเดียวกับที่ตั้งไว้ในเว็บผู้เช่า node scripts/generate-license-key.js "ชื่อผู้เช่า" 30
```

(`30` = จำนวนวันที่เช่า) จะได้คีย์รูปแบบ `LTS1-xxxxx.yyyyy` ส่งให้ผู้เช่าไปกรอกที่หน้า `/license` ของเว็บเขา ปลดล็อกใช้งานได้ทันที หมดอายุแล้วเว็บจะล็อกอัตโนมัติ ต่ออายุ = ออกคีย์ใหม่ให้อีกครั้ง

**3. หลังจากนั้นผู้เช่าตั้งค่าร้านเอง** ทั้งหมดผ่านหน้า `/admin` ที่มีอยู่แล้ว (ชื่อร้าน โลโก้ บัญชี/QR รับเงิน สินค้า มินิเกม ฯลฯ) ไม่ต้องพึ่งคุณอีก ยกเว้นเรื่องต่ออายุคีย์
